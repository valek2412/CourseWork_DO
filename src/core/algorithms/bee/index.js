...
const generateStates = (initialState, suitsTotalCount, amountStates) => {
    return [...Array(amountStates)].map(() =>
        createState(initialState, suitsTotalCount)
    );
};

const beeAlgorithm = (initialState, suitsTotalCount, iterations, amountStartStates, amountBestStates) => {
    const before = new Date();

    const beforeInitialStates = new Date();
    const initialStates = generateStates(initialState, suitsTotalCount, amountStartStates);
    const afterInitialStates = new Date();

    const sortStates = initialStates.sort((a, b) => a.fitnessValue - b.fitnessValue);
...
const wrapper = (initialState, suitsTotalCount, iterations, amountStartStates = 16, amountBestStates = 1) => {
    ...
    console.info(`initialStatesDiff: ${metaData.initialPopulationDiff} ms`);
